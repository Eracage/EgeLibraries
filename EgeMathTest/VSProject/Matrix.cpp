...
TEST_CASE("Matrix2 tests", "[matrix]")
{
	...
	SECTION("Operators")
	{
		...
		CHECK(Matrix2<int>(Mat2(1, 2, 3, 4)) == Matrix2<int>(1, 2, 3, 4));
	}

	SECTION("Inverse, transpose")
	{
		Mat2 mat(1, 2, 3, 4);
		CHECK(mat.inversed() == (0.5f*Mat2(-4, 2, 3, -1)));

		CHECK(mat.transposed() == Mat2(1, 3, 2, 4));
	}
}
...
TEST_CASE("Matrix3 tests", "[matrix]")
{
	...
	SECTION("Operators")
	{
		...
		CHECK(Matrix3<int>(Mat3(Vec3(1), Vec3(2), Vec3(3))) 
			== Matrix3<int>(Vec3(1), Vec3(2), Vec3(3)));
	}
	...
	SECTION("Inverse, transpose, etc...")
	{
		...
		CHECK(mat.inversed() == (1.f/3.f*Mat3(3, -6, 3, -6, 3, 0, 3, 2, -2)));

		CHECK(mat.transposed() == Mat3(2, 4, 7, 2, 5, 8, 3, 6, 9));

		CHECK(mat.subMatrix2(2, 2) == Mat2(2,2, 4,5));
	}
}
...
TEST_CASE("Matrix4 tests", "[matrix]")
{
	...
	SECTION("Operators")
	{
		...
		CHECK((mat1 / 2) == Mat4(.5f, 1, 1.5f, 2,
			...
			4.5f, 5, 5.5f, 6,
			6.5f, 7, 7.5f, 8));
		CHECK(Matrix4<int>(Mat4(Vec4(1), Vec4(2), Vec4(3), Vec4(4)))
			== Matrix4<int>(Vec4(1), Vec4(2), Vec4(3), Vec4(4)));
	}
	...
	SECTION("Inverse, transpose, etc..")
	{
		...
		CHECK(mat.inversed() == (1.f/2580.f * Mat4(-54, -330, 42, 408,
			-196, 140, 248, -48,
			68, 320, -244, -36, 
			461, -145, 167, -282)));

		CHECK(mat.transposed() == Mat4(5, 3, 4, 9,
			2, 7, 9, 5,
			7, 9, 2, 8,
			6, 2, 4, 2));

		CHECK(mat.subMatrix3(2,2) == Mat3(5,2,6,
			3,7,2,
			9,5,2));
	}
